...
@section('content')
    <div class="card">
        <div class="card-header">
            <h3 class="card-title card_title_center"> {{ __('messages.Add_New') }}  {{ __('messages.parentStudents') }} </h3>
        </div>
        <!-- /.card-header -->
        <div class="card-body">


            <form action="{{ route('parentStudents.store') }}" method="post" enctype='multipart/form-data'>
                <div class="row">
                    @csrf
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input name="email" id="notes" class="form-control" value="{{ old('email') }}">
                            @error('email')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input name="username" id="notes" class="form-control" value="{{ old('username') }}">
                            @error('username')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
