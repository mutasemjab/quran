...
@section('title')
    {{ __('messages.Edit') }} {{ __('messages.students') }}
@endsection



@section('contentheaderlink')
    <a href="{{ route('students.index') }}"> {{ __('messages.students') }} </a>
@endsection
...
@section('content')
    <div class="card">
        <div class="card-header">
            <h3 class="card-title card_title_center"> {{ __('messages.Edit') }} {{ __('messages.students') }} </h3>
        </div>
        <!-- /.card-header -->
        <div class="card-body">
                ...
                <form action="{{ route('students.update', $data['id']) }}" method="POST" enctype='multipart/form-data'>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" name="email" id="name" class="form-control" value="{{ old('email',$data['email']) }}">
                            @error('email')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input name="phone" id="notes" class="form-control" value="{{ old('phone', $data['phone']) }}">
                            @error('phone')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="form-group col-md-6">
                        <label for="classes">{{ __('messages.classes') }}</label>
                        <select class="form-control" name="class" id="classes">
                            <option value="">Select Class</option>
                            @foreach($classes as $class)
                            <option value="{{ $class->id }}" {{ $class->id == $data->clas_id ? 'selected' : '' }}>
                                {{ $class->name }}
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-12">
                        <div class="form-group text-center">
                            ...
                            <a href="{{ route('students.index') }}" class="btn btn-sm btn-danger">{{ __('messages.Cancel') }}</a>
